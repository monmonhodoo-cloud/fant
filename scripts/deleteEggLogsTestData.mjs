...
async function deleteInBatches(db, docs) {
  ...
  for (let i = 0; i < docs.length; i += BATCH_LIMIT) {
    ...
    console.log(`[egglogs-delete] deleted ${deleted}/${docs.length}`);
  }
}
...
async function main() {
  ...
  console.log(`[egglogs-delete] mode: ${execute ? 'execute' : 'dry-run'}`);
  console.log(`[egglogs-delete] project: ${PROJECT_ID}`);
  console.log(`[egglogs-delete] ${COLLECTION}: ${docs.length}`);
  for (const [date, count] of summarizeDocs(docs).slice(0, 50)) {
    console.log(`  - ${date}: ${count}`);
  }

  const backupPath = `egglogs-delete-backup-${timestampForPath()}.json`;
  await fs.writeFile(
    backupPath,
    JSON.stringify({
  ...
  console.log(`[egglogs-delete] backup written: ${backupPath}`);

  if (!execute) {
    console.log('[egglogs-delete] dry-run only. No documents deleted.');
    return;
  }

  await deleteInBatches(db, docs);
  console.log('[egglogs-delete] done');
}

main().catch((err) => {
  console.error('[egglogs-delete] failed:', err);
  process.exit(1);
});
